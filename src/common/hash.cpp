...
int MD5::update(CFile *pfile) 
{
	...
	int nread_bytes;
	int64_t nleft_bytes;
	char buffer[KL_COMMON_BUF_SIZE];

	nleft_bytes = pfile->get_file_size();
	while (nleft_bytes > 0) 
	{
		nread_bytes = nleft_bytes > KL_COMMON_BUF_SIZE ? KL_COMMON_BUF_SIZE : \
			nleft_bytes;
		nread_bytes = pfile->read_file(buffer, nread_bytes);
		if (nread_bytes < 0) 
		{
			return errno;
		}
		update((const byte*)buffer, nread_bytes);
		nleft_bytes -= nread_bytes;
	}
	return 0;
}
...
int MD5::update(const byte* input, size_t length) 
{
	...
	if (length >= part_len) 
	{
		memcpy(&m_buffer[index], input, part_len);
		transform(m_buffer);
...
int MD5::final() 
{
	...
	memcpy(m_state, old_state, 16);
	memcpy(m_count, old_count, 8);
	return 0;
}
